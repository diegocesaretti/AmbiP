...
public final class MainActivity extends ComponentActivity {
    ...
    private void buildUi(){
        ...
        int cw=Math.min(sw,Math.round(sh*4f/3f)),ch=Math.min(sh,Math.round(cw*3f/4f));
        ...
        calibrationContainer.addView(previewView,new FrameLayout.LayoutParams(-1,-1));
    ...
    private void buildCalibrationToolbar(){
        ...
        calibrationToolbar.addView(makeButton("ZOOM +",v->changeZoom(0.25f)));
        calibrationToolbar.addView(makeButton("RESET",v->{float[] c={0.10f,0.14f,0.90f,0.14f,0.90f,0.86f,0.10f,0.86f};calibrationView.setCorners(c);if(analyzer!=null)analyzer.setCorners(c);}));
        calibrationToolbar.addView(makeButton("DONE",v->exitCalibration()));
        FrameLayout.LayoutParams tb=new FrameLayout.LayoutParams(-2,-2);tb.gravity=Gravity.TOP|Gravity.CENTER_HORIZONTAL;tb.topMargin=dp(8);calibrationToolbar.setVisibility(View.GONE);root.addView(calibrationToolbar,tb);
    }

    private void buildSettingsPanel(){
        ScrollView scroll=new ScrollView(this);scroll.setFillViewport(true);scroll.setBackgroundColor(0xE6000000);
        settingsPanel=new LinearLayout(this);settingsPanel.setOrientation(LinearLayout.VERTICAL);settingsPanel.setPadding(dp(18),dp(14),dp(18),dp(18));
        ...
        TextView hint=new TextView(this);hint.setText("Camera controls affect capture. Ambient controls affect only projected light.");hint.setTextColor(0xFFBBBBBB);hint.setTextSize(12f);hint.setPadding(0,0,0,dp(12));settingsPanel.addView(hint);
        ...
        addExposureControl();
        addZoomControl();
        addFloatSlider("Capture contrast",0.45f,2.20f,1f,"captureContrast",v->{if(analyzer!=null)analyzer.setCaptureContrast(v);});
        addFloatSlider("Capture saturation",0f,2.20f,1f,"captureSaturation",v->{if(analyzer!=null)analyzer.setCaptureSaturation(v);});

        addSection("AMBIENT PROJECTION");
        addFloatSlider("Ambient brightness",0.15f,2f,1f,"ambientBrightness",v->{if(analyzer!=null)analyzer.setAmbientBrightness(v);});
        addFloatSlider("Ambient contrast",0.45f,2.20f,1f,"ambientContrast",v->{if(analyzer!=null)analyzer.setAmbientContrast(v);});
        addTemperatureSlider();
        addFloatSlider("Motion smoothing",0f,0.96f,0.68f,"smoothing",v->{if(analyzer!=null)analyzer.setSmoothing(v);});
        ...
        FrameLayout.LayoutParams sp=new FrameLayout.LayoutParams(Math.min(dp(430),Math.round(getResources().getDisplayMetrics().widthPixels*0.48f)),-1);sp.gravity=Gravity.END;scroll.setVisibility(View.GONE);scroll.setTag("settingsScroll");root.addView(scroll,sp);
    }
    ...
    private void addTemperatureSlider(){
        ...
        float initial=prefs==null?0f:prefs.getFloat("ambientColorTemperature",0f);
        label.setText(temperatureLabel(initial));box.addView(label);
        SeekBar seek=new SeekBar(this);seek.setMax(1000);seek.setProgress(Math.round((initial+1f)*500f));box.addView(seek,new LinearLayout.LayoutParams(-1,dp(40)));
        seek.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener(){
            @Override public void onProgressChanged(SeekBar s,int progress,boolean fromUser){float v=progress/500f-1f;label.setText(temperatureLabel(v));if(analyzer!=null)analyzer.setAmbientColorTemperature(v);if(fromUser&&prefs!=null)prefs.edit().putFloat("ambientColorTemperature",v).apply();}
            @Override public void onStartTrackingTouch(SeekBar s){}
            @Override public void onStopTrackingTouch(SeekBar s){}
        });
        if(analyzer!=null)analyzer.setAmbientColorTemperature(initial);settingsPanel.addView(box);
    }

    private String temperatureLabel(float v){
        if(Math.abs(v)<0.04f)return "Color temperature: Neutral";
        return String.format(Locale.US,"Color temperature: %s %.0f%%",v<0?"Warm":"Cool",Math.abs(v)*100f);
    }

    private void addSection(String text){TextView v=new TextView(this);v.setText(text);v.setTextColor(Color.WHITE);v.setTextSize(13f);v.setPadding(0,dp(16),0,dp(5));settingsPanel.addView(v);}
    ...
    private void addExposureControl(){
        ...
        exposureSeek=new SeekBar(this);exposureSeek.setMax(1000);exposureSeek.setProgress(500);settingsPanel.addView(exposureSeek,new LinearLayout.LayoutParams(-1,dp(40)));
        exposureSeek.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener(){@Override public void onProgressChanged(SeekBar s,int p,boolean f){if(f)applyExposureFromSlider(p,true);}@Override public void onStartTrackingTouch(SeekBar s){}@Override public void onStopTrackingTouch(SeekBar s){}});
    }
    ...
    private void addZoomControl(){
        ...
        zoomSeek=new SeekBar(this);zoomSeek.setMax(1000);zoomSeek.setProgress(115);settingsPanel.addView(zoomSeek,new LinearLayout.LayoutParams(-1,dp(40)));
        zoomSeek.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener(){@Override public void onProgressChanged(SeekBar s,int p,boolean f){if(f)applyZoomFromSlider(p,true);}@Override public void onStartTrackingTouch(SeekBar s){}@Override public void onStopTrackingTouch(SeekBar s){}});
    }
    ...
    private void addFloatSlider(String title,float min,float max,float def,String key,FloatConsumer consumer){
        LinearLayout box=new LinearLayout(this);box.setOrientation(LinearLayout.VERTICAL);TextView label=new TextView(this);label.setTextColor(Color.WHITE);label.setTextSize(14f);
        ...
        SeekBar seek=new SeekBar(this);seek.setMax(1000);seek.setProgress(Math.round((initial-min)/(max-min)*1000f));box.addView(seek,new LinearLayout.LayoutParams(-1,dp(40)));
        seek.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener(){@Override public void onProgressChanged(SeekBar s,int p,boolean f){float v=min+(max-min)*(p/1000f);label.setText(String.format(Locale.US,"%s: %.2f",title,v));consumer.accept(v);if(f&&prefs!=null)prefs.edit().putFloat(key,v).apply();}@Override public void onStartTrackingTouch(SeekBar s){}@Override public void onStopTrackingTouch(SeekBar s){}});
        consumer.accept(initial);settingsPanel.addView(box);
    }

    private void restoreSettings(){
        if(prefs==null||analyzer==null)return;
        analyzer.setCaptureContrast(prefs.getFloat("captureContrast",1f));analyzer.setCaptureSaturation(prefs.getFloat("captureSaturation",1f));
        ...
        analyzer.setSmoothing(prefs.getFloat("smoothing",0.68f));ambilightView.setOuterFadeRatio(prefs.getFloat("outerFade",0.16f));calibrationZoom=prefs.getFloat("cameraZoom",1.8f);requestedExposureIndex=prefs.getInt("exposureIndex",0);
    }

    private void resetSettings(){
        if(prefs!=null)prefs.edit().clear().apply();
        if(analyzer!=null){analyzer.setCaptureContrast(1f);analyzer.setCaptureSaturation(1f);analyzer.setAmbientBrightness(1f);analyzer.setAmbientContrast(1f);analyzer.setAmbientColorTemperature(0f);analyzer.setSmoothing(0.68f);}
    ...
    private void toggleSettings(){settingsVisible=!settingsVisible;View scroll=root.findViewWithTag("settingsScroll");if(scroll!=null)scroll.setVisibility(settingsVisible?View.VISIBLE:View.GONE);if(!settingsVisible&&previewVisible){calibrationContainer.setVisibility(View.GONE);previewVisible=false;}}
    ...
    private TextView makeButton(String text,View.OnClickListener listener){TextView b=new TextView(this);b.setText(text);b.setTextColor(Color.WHITE);b.setTextSize(13f);b.setGravity(Gravity.CENTER);b.setPadding(dp(11),dp(9),dp(11),dp(9));b.setBackgroundColor(0xAA333333);LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-2,-2);p.setMargins(dp(3),dp(2),dp(3),dp(2));b.setLayoutParams(p);b.setOnClickListener(listener);return b;}

    private void enterCalibration(){if(analyzer==null)return;calibrating=true;previewVisible=true;settingsVisible=false;View scroll=root.findViewWithTag("settingsScroll");if(scroll!=null)scroll.setVisibility(View.GONE);applyZoom(calibrationZoom);calibrationContainer.setVisibility(View.VISIBLE);calibrationView.setCorners(analyzer.getCorners());calibrationView.setVisibility(View.VISIBLE);calibrationToolbar.setVisibility(View.VISIBLE);ambilightView.setVisibility(View.GONE);Toast.makeText(this,"Zoomed calibration · use −/+ if needed, then drag all 4 corners",Toast.LENGTH_LONG).show();}
    ...
    private void applyZoom(float requested){if(boundCamera==null)return;try{androidx.camera.core.ZoomState zs=boundCamera.getCameraInfo().getZoomState().getValue();float max=zs==null?requested:zs.getMaxZoomRatio(),min=zs==null?1f:zs.getMinZoomRatio();calibrationZoom=Math.max(min,Math.min(max,requested));boundCamera.getCameraControl().setZoomRatio(calibrationZoom);if(prefs!=null)prefs.edit().putFloat("cameraZoom",calibrationZoom).apply();if(zoomLabel!=null)zoomLabel.setText(String.format(Locale.US,"Camera zoom: %.2fx",calibrationZoom));}catch(Throwable t){Log.w(TAG,"Zoom not available",t);}}
    private void applyZoomFromSlider(int progress,boolean persist){if(boundCamera==null)return;try{androidx.camera.core.ZoomState zs=boundCamera.getCameraInfo().getZoomState().getValue();if(zs==null)return;float min=zs.getMinZoomRatio(),max=zs.getMaxZoomRatio(),ratio=min+(max-min)*(progress/1000f);calibrationZoom=ratio;boundCamera.getCameraControl().setZoomRatio(ratio);zoomLabel.setText(String.format(Locale.US,"Camera zoom: %.2fx",ratio));if(persist&&prefs!=null)prefs.edit().putFloat("cameraZoom",ratio).apply();}catch(Throwable t){Log.w(TAG,"Zoom slider failed",t);}}
    ...
    private void applyExposureFromSlider(int progress,boolean persist){if(boundCamera==null)return;try{ExposureState es=boundCamera.getCameraInfo().getExposureState();Range<Integer> range=es.getExposureCompensationRange();int min=range.getLower(),max=range.getUpper(),idx=Math.round(min+(max-min)*(progress/1000f));applyExposureIndex(idx,persist);}catch(Throwable t){Log.w(TAG,"Exposure slider failed",t);}}
    private void applyExposureIndex(int requested,boolean persist){if(boundCamera==null)return;try{ExposureState es=boundCamera.getCameraInfo().getExposureState();Range<Integer> range=es.getExposureCompensationRange();int idx=Math.max(range.getLower(),Math.min(range.getUpper(),requested));requestedExposureIndex=idx;if(es.isExposureCompensationSupported())boundCamera.getCameraControl().setExposureCompensationIndex(idx);if(exposureLabel!=null)exposureLabel.setText("Exposure compensation: "+idx);if(persist&&prefs!=null)prefs.edit().putInt("exposureIndex",idx).apply();syncExposureSlider();}catch(Throwable t){Log.w(TAG,"Exposure not available",t);}}
    private void syncExposureSlider(){if(boundCamera==null||exposureSeek==null)return;try{ExposureState es=boundCamera.getCameraInfo().getExposureState();Range<Integer> range=es.getExposureCompensationRange();int min=range.getLower(),max=range.getUpper();exposureSeek.setEnabled(es.isExposureCompensationSupported()&&max>min);float p=max==min?0.5f:(requestedExposureIndex-min)/(float)(max-min);exposureSeek.setProgress(Math.round(Math.max(0f,Math.min(1f,p))*1000f));exposureLabel.setText(es.isExposureCompensationSupported()?"Exposure compensation: "+requestedExposureIndex:"Exposure compensation: unsupported");}catch(Throwable ignored){}}
