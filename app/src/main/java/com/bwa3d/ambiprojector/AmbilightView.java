...
/** Projection output layer: continuous ambient halo, smooth corners, outer vignette and text zones. */
public final class AmbilightView extends View {
    ...
    @Override protected void onDraw(@NonNull Canvas canvas){
        ...
        canvas.drawColor(Color.BLACK);RectF tv=tvMask(w,h);
        drawContinuousSides(canvas,tv);

        // Corners use several neighbouring samples and three overlapping soft glows.
        // This makes light "turn the corner" instead of forming a separate radial blob.
        drawSmoothCorner(canvas,tv,true,true,
                edgeAverage(state.top,true),edgeAverage(state.left,true));
        drawSmoothCorner(canvas,tv,false,true,
                edgeAverage(state.top,false),edgeAverage(state.right,true));
        drawSmoothCorner(canvas,tv,true,false,
                edgeAverage(state.bottom,true),edgeAverage(state.left,false));
        drawSmoothCorner(canvas,tv,false,false,
                edgeAverage(state.bottom,false),edgeAverage(state.right,false));
    ...
    private void drawSmoothCorner(Canvas c,RectF tv,boolean left,boolean top,int horizontalColor,int verticalColor){
        ...
        int blended=mix(horizontalColor,verticalColor);

        // Central broad bridge.
        drawSoftGlow(c,left,top,x,y,radius,blended,168);
        // Extend the top/bottom edge colour into the corner.
        drawSoftGlow(c,left,top,x+sx*ex*0.20f,y,radius*0.92f,horizontalColor,92);
        // Extend the left/right edge colour into the corner.
    ...
    private void drawSoftGlow(Canvas c,boolean left,boolean top,float cx,float cy,float radius,int color,int alpha){
        ...
        int mid=withAlpha(color,Math.round(alpha*0.56f));
        int faint=withAlpha(color,Math.round(alpha*0.18f));
        ...
        float l=left?0f:Math.min(getWidth(),cx),r=left?Math.max(0f,cx):getWidth();
        float t=top?0f:Math.min(getHeight(),cy),b=top?Math.max(0f,cy):getHeight();
        c.drawRect(l,t,r,b,paint);
    }

    /** Average a few neighbouring edge samples so one hot pixel/zone cannot dominate a corner. */
    private int edgeAverage(int[] src,boolean first){
        if(src==null||src.length==0)return Color.BLACK;
        int n=Math.min(4,src.length),rs=0,gs=0,bs=0;
        for(int i=0;i<n;i++){int idx=first?i:src.length-1-i,c=src[idx];rs+=(c>>16)&255;gs+=(c>>8)&255;bs+=c&255;}
        return 0xFF000000|((rs/n)<<16)|((gs/n)<<8)|(bs/n);
    }

    /** Blend several end points instead of modifying only the single final gradient stop. */
    private int[] softCornerEnds(int[] src,int first,int last){
        int[] out=src.clone();int n=Math.min(4,out.length);
        for(int i=0;i<n;i++){
            float strength=(n-i)/(float)(n+1);out[i]=mixWeighted(out[i],first,strength*0.55f);
            int j=out.length-1-i;out[j]=mixWeighted(out[j],last,strength*0.55f);
        }
        return out;
    }
